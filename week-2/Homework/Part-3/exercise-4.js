  ...
  let restaurantFinderApplication = {
    ...
    findAvailableRestaurants: function (numberOfPeople) {
        ...
        restaurants.forEach(element => {
            if ((element.totalSeats - element.numberOfCustomers) >= numberOfPeople) {
                availableRestaurants.push(element.name)
            }
        });
    ...
    findRestaurantServingDish: function (dishName){
      ...
      restaurants.forEach.(dish => dish.menu.includes(dishName)).map(restaurents.restaurants.name);
      // Complete here
    }
    ...
    countNumberOfRestaurantsInArea: function (area) {
      ...
      let count = 0;
      restaurants.forEach(element => {
          if (element.address.area === "center") {
              count += 1
          }
      })
      return count;
